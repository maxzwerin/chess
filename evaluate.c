...
void syncMailbox(Board *b) {
    for (int sq = 0; sq < 64; sq++) board[sq] = EMPTY;

    U64 bb;

    // Pawns
    bb = b->pawnBB & b->whiteBB;
    while (bb) board[popLSB(&bb)] = WHITE_PAWN;

    bb = b->pawnBB & b->blackBB;
    while (bb) board[popLSB(&bb)] = BLACK_PAWN;

    // Knights
    bb = b->knightBB & b->whiteBB;
    while (bb) board[popLSB(&bb)] = WHITE_KNIGHT;

    bb = b->knightBB & b->blackBB;
    while (bb) board[popLSB(&bb)] = BLACK_KNIGHT;

    // Bishops
    bb = b->bishopBB & b->whiteBB;
    while (bb) board[popLSB(&bb)] = WHITE_BISHOP;

    bb = b->bishopBB & b->blackBB;
    while (bb) board[popLSB(&bb)] = BLACK_BISHOP;

    // Rooks
    bb = b->rookBB & b->whiteBB;
    while (bb) board[popLSB(&bb)] = WHITE_ROOK;

    bb = b->rookBB & b->blackBB;
    while (bb) board[popLSB(&bb)] = BLACK_ROOK;

    // Queens
    bb = b->queenBB & b->whiteBB;
    while (bb) board[popLSB(&bb)] = WHITE_QUEEN;

    bb = b->queenBB & b->blackBB;
    while (bb) board[popLSB(&bb)] = BLACK_QUEEN;

    // Kings
    bb = b->kingBB & b->whiteBB;
    while (bb) board[popLSB(&bb)] = WHITE_KING;

    bb = b->kingBB & b->blackBB;
    while (bb) board[popLSB(&bb)] = BLACK_KING;
...
int search(Board *board, int depth, int alpha, int beta) {
    ...
    for (int i = 0; i < moveCount; i++) {
        ...
        Board cpy = *board;
        makeMove(&cpy, moves[i]);

        int score = -search(&cpy, depth - 1, -beta, -alpha);
...
Move findBestMove(Board *board, int depth) {
    ...
    for (int i = 0; i < moveCount; i++) {
        ...
        int eval = -search(&cpy, depth - 1, -INF, INF);
