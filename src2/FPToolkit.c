...
int G_wait_event(double p[2])
// sig <= -1000 means nothing has happened yet, still waiting for event
// -3, -4.. etc. indicate mouse event, otherwise return non-negative ascii value
// see function Handle_Events_X() [~line 550] for specific sig numbers
{
    int sig;
    int pi[2];
...
int G_key_press() {
    ...
    while (XPending(XxDisplay)) {
        ...
        //     KeySym keysym = XLookupKeysym(&event.xkey, 0);
        //     if ((int)keysym == held_key) {
        //         held_key = -1; // Key was released
        //     }
        // }
    }
...
/* Carl Swinford -- April 24, 2024 ================================================
  Here's some supplemental functions I made for David Ely's library. The different
  functions summarize to include:
    - Getters for display and window values.
    - Function that gets mouse coordinates regardless of whether the mouse has 
    clicked or not (will also return the state of each button on the mouse).
    - State checker for keyboard input that doesn't block.

  (NOTE: keyboard and mouse state functions don't update image buffer to screen
   like G_wait_click, this will manually need to be done if waiting for an input
   by calling the function G_display_image)
*/

// Gives coder access to the display pointer if they want to do LibX11 stuff
Display * S_get_display_ptr() { return XxDisplay; }

// Gives coder access to the window value if they want to do LibX11 stuff
Window S_get_window_ptr() { return XxWindow; }

// Fills array of size 2 with x and y position of mouse cursor (without clicking)
// Returns mouse button states in bit order: {LEFTCLICK, MIDDLECLICK, RIGHTCLICK}
// Coords[0] = x, Coords[1] = y 
/* int Relation: 
  0 = Mouse coords relative to the entire display 
  1 = Mouse coords relative to the window 

  (0 and 1 are not guaranteed to track outside of window)
*/
int S_mouse_coord(int * Coords, int Relation) {
  int ReturnVal = -1;

  Window FocusReturn = -1;
  int RevertToReturn = -1;
  ...
  XQueryPointer(
    ...
    ReturnVal = (int)(MouseEvent.xbutton.state >> 8);

  switch (Relation) {
    case 0: // Mouse coords relative to entire screen
      Coords[0] = MouseEvent.xbutton.x_root;
      Coords[1] = MouseEvent.xbutton.y_root;
      break;
    case 1: // Mouse coords relative to Window coords
      Coords[0] = MouseEvent.xbutton.x;
      Coords[1] = MouseEvent.xbutton.y;
      break;
    default:
      ReturnVal = -1;
      break;
...
// repackages S_mouse_coord function
int S_mouse_coord_screen(int * Coords) { return S_mouse_coord(Coords, 0); }
int S_mouse_coord_window(int * Coords) { return S_mouse_coord(Coords, 1); }

// Fills 32 byte char array with raw key states in the form of individual bits
// The keys correlated with each bit can be found below S_key_down
char * S_key_state(char * KeyReturn) {
    XQueryKeymap(XxDisplay, KeyReturn);
    return KeyReturn;
}


// Returns the state of the key that was passed in for KeyCheck
// Uses key values that can be obatined from G_wait_key
int S_key_down(int KeyCheck) {
  char KeyReturn[32] = { 0 };
  unsigned short KeyToShort = 0;

  XQueryKeymap(XxDisplay, KeyReturn);

  // ESC
  if (KeyCheck == 0xff1b) return (KeyReturn[1] & (1 << 1)) ? 1 : 0;

  // Number row
  if (KeyCheck >= 0x30 && KeyCheck <= 0x39) {
    KeyCheck -= 0x30;

    KeyToShort = *((unsigned short *)&(KeyReturn[1]));
    KeyToShort >>= 1;

    if (!KeyCheck) return (KeyToShort & (1 << 10)) ? 1 : 0;
    return (KeyToShort & (1 << KeyCheck)) ? 1 : 0;
  }

  // Minus on row, Equal sign on row, BACKSPACE, TAB
  if (KeyCheck == 0x2d) return (KeyReturn[2] >> 4) & 1;
  if (KeyCheck == 0x3d) return (KeyReturn[2] >> 5) & 1;
  if (KeyCheck == 0xff08) return (KeyReturn[2] >> 6) & 1;
  if (KeyCheck == 0xff09) return (KeyReturn[2] >> 7) & 1;
  
  // Q, W, E, R, T, Y, U, I
  if (KeyCheck == 0x71) return (KeyReturn[3] >> 0) & 1;
  if (KeyCheck == 0x77) return (KeyReturn[3] >> 1) & 1;
  if (KeyCheck == 0x65) return (KeyReturn[3] >> 2) & 1;
  if (KeyCheck == 0x72) return (KeyReturn[3] >> 3) & 1;
  if (KeyCheck == 0x74) return (KeyReturn[3] >> 4) & 1;
  if (KeyCheck == 0x79) return (KeyReturn[3] >> 5) & 1;
  if (KeyCheck == 0x75) return (KeyReturn[3] >> 6) & 1;
  if (KeyCheck == 0x69) return (KeyReturn[3] >> 7) & 1;
  
  // O, P, [, ], ENTER, LCTRL, A, S
  if (KeyCheck == 0x6f) return (KeyReturn[4] >> 0) & 1;
  if (KeyCheck == 0x70) return (KeyReturn[4] >> 1) & 1;
  if (KeyCheck == 0x5b) return (KeyReturn[4] >> 2) & 1;
  if (KeyCheck == 0x5d) return (KeyReturn[4] >> 3) & 1;
  if (KeyCheck == 0xff0d) return (KeyReturn[4] >> 4) & 1;
  if (KeyCheck == 0xffe3) return (KeyReturn[4] >> 5) & 1;
  if (KeyCheck == 0x61) return (KeyReturn[4] >> 6) & 1;
  if (KeyCheck == 0x73) return (KeyReturn[4] >> 7) & 1;
  
  // D, F, G, H, J, K, L, ;
  if (KeyCheck >= 0x64 && KeyCheck <= 0x6c) {
    if (KeyCheck > 0x64) {
      if (KeyCheck > 0x68) --KeyCheck;
      --KeyCheck;
    }
    KeyCheck -= 0x64;
    
    KeyToShort = *((unsigned short *)&(KeyReturn[5]));

    return (KeyToShort & (1 << KeyCheck)) ? 1 : 0;
  }
  if (KeyCheck == 0x3b) return (KeyReturn[5] >> 7) & 1;

  // ', `, LSHIFT, , Z, X, C, V
  if (KeyCheck == 0x27) return (KeyReturn[6] >> 0) & 1;
  if (KeyCheck == 0x60) return (KeyReturn[6] >> 1) & 1;
  if (KeyCheck == 0xffe1) return (KeyReturn[6] >> 2) & 1;
  //if (KeyCheck == NULL) return (KeyReturn[6] >> 3) & 1;
  if (KeyCheck == 0x7a) return (KeyReturn[6] >> 4) & 1;
  if (KeyCheck == 0x78) return (KeyReturn[6] >> 5) & 1;
  if (KeyCheck == 0x63) return (KeyReturn[6] >> 6) & 1;
  if (KeyCheck == 0x76) return (KeyReturn[6] >> 7) & 1;

  // B, N, M, <, >, /, RSHIFT, NUM*
  if (KeyCheck == 0x62) return (KeyReturn[7] >> 0) & 1;
  if (KeyCheck == 0x6e) return (KeyReturn[7] >> 1) & 1;
  if (KeyCheck == 0x6d) return (KeyReturn[7] >> 2) & 1;
  if (KeyCheck == 0x2c) return (KeyReturn[7] >> 3) & 1;
  if (KeyCheck == 0x2e) return (KeyReturn[7] >> 4) & 1;
  if (KeyCheck == 0x2f) return (KeyReturn[7] >> 5) & 1;
  if (KeyCheck == 0xffe2) return (KeyReturn[7] >> 6) & 1;
  if (KeyCheck == 0xffaa) return (KeyReturn[7] >> 7) & 1;
  
  // LALT, SPACE, ,
  if (KeyCheck == 0xffe9) return (KeyReturn[8] >> 0) & 1;
  if (KeyCheck == 0x20) return (KeyReturn[8] >> 1) & 1;
  //if (KeyCheck == NULL) return (KeyReturn[8] >> 2) & 1;

  // F1 - F10
  if (KeyCheck >= 0xffbe && KeyCheck <= 0xffc7) {
    KeyCheck -= 0xffbe;

    KeyToShort = *((unsigned short *)&(KeyReturn[8]));
    KeyToShort >>= 3;

    return (KeyToShort & (1 << KeyCheck)) ? 1 : 0;
  }

  // NUMLOCK, LOCKSCROLL, NUM7
  if (KeyCheck == 0xff7f) return (KeyReturn[9] >> 5) & 1;
  if (KeyCheck == 0xff14) return (KeyReturn[9] >> 6) & 1;
  if (KeyCheck == 0xff95) return (KeyReturn[9] >> 7) & 1;

  // NUM8, NUM9, NUM-, NUM4, NUM5, NUM6, NUM+, NUM1
  if (KeyCheck == 0xffb8) return (KeyReturn[10] >> 0) & 1;
  if (KeyCheck == 0xffb9) return (KeyReturn[10] >> 1) & 1;
  if (KeyCheck == 0xffad) return (KeyReturn[10] >> 2) & 1;
  if (KeyCheck == 0xffb4) return (KeyReturn[10] >> 3) & 1;
  if (KeyCheck == 0xffb5) return (KeyReturn[10] >> 4) & 1;
  if (KeyCheck == 0xffb6) return (KeyReturn[10] >> 5) & 1;
  if (KeyCheck == 0xffab) return (KeyReturn[10] >> 6) & 1;
  if (KeyCheck == 0xffb1) return (KeyReturn[10] >> 7) & 1;

  // NUM2, NUM3, NUM0, NUM., , , , F11
  if (KeyCheck == 0xffb2) return (KeyReturn[11] >> 0) & 1;
  if (KeyCheck == 0xffb3) return (KeyReturn[11] >> 1) & 1;
  if (KeyCheck == 0xffb0) return (KeyReturn[11] >> 2) & 1;
  if (KeyCheck == 0xffae) return (KeyReturn[11] >> 3) & 1;
  //if (KeyCheck == NULL) return (KeyReturn[11] >> 4) & 1;
  //if (KeyCheck == NULL) return (KeyReturn[11] >> 5) & 1;
  //if (KeyCheck == NULL) return (KeyReturn[11] >> 6) & 1;
  if (KeyCheck == 0xffc8) return (KeyReturn[11] >> 7) & 1;

  // F12
  if (KeyCheck == 0xffc9) return (KeyReturn[12] >> 0) & 1;

  // NUMENTER, RCTRL, NUM/, , RALT, , HOME, UPARR
  if (KeyCheck == 0xff8d) return (KeyReturn[13] >> 0) & 1;
  if (KeyCheck == 0xffe4) return (KeyReturn[13] >> 1) & 1;
  if (KeyCheck == 0xffaf) return (KeyReturn[13] >> 2) & 1;
  //if (KeyCheck == NULL) return (KeyReturn[13] >> 3) & 1;
  if (KeyCheck == 0xffea) return (KeyReturn[13] >> 4) & 1;
  //if (KeyCheck == NULL) return (KeyReturn[13] >> 5) & 1;
  if (KeyCheck == 0xff50) return (KeyReturn[13] >> 6) & 1;
  if (KeyCheck == 0xff52) return (KeyReturn[13] >> 7) & 1;

  // PAGEUP, LEFTARR, RIGHTARR, END, DOWNARR, PAGEDOWN, INSERT, DELETE
  if (KeyCheck == 0xff55) return (KeyReturn[14] >> 0) & 1;
  if (KeyCheck == 0xff51) return (KeyReturn[14] >> 1) & 1;
  if (KeyCheck == 0xff53) return (KeyReturn[14] >> 2) & 1;
  if (KeyCheck == 0xff57) return (KeyReturn[14] >> 3) & 1;
  if (KeyCheck == 0xff54) return (KeyReturn[14] >> 4) & 1;
  if (KeyCheck == 0xff56) return (KeyReturn[14] >> 5) & 1;
  if (KeyCheck == 0xff63) return (KeyReturn[14] >> 6) & 1;
  if (KeyCheck == 0xffff) return (KeyReturn[14] >> 7) & 1;

  if (KeyCheck == 0xff13) return (KeyReturn[15] >> 7) & 1;

  return 0;
}

/* KeyReturn: bit state values from XQueryKeymap
  [0] = {,,,,,,,}
  [1] = {,ESC,1,2,3,4,5,6}
  [2] = {7,8,9,0,-,=,BACKSPACE,TAB}
  [3] = {Q,W,E,R,T,Y,U,I}
  [4] = {O,P,[,],ENTER,LCTRL,A,S}
  [5] = {D,F,G,H,J,K,L,;}
  [6] = {',`,LSHIFT,,Z,X,C,V}
  [7] = {B,N,M,<,>,/,RSHIFT,NUM*}

  [8] = {LALT,SPACE,,F1,F2,F3,F4,F5}
  [9] = {F6,F7,F8,F9,F10,NUMLOCK,LOCKSCROLL,NUM7}
  [10] = {NUM8,NUM9,NUM-,NUM4,NUM5,NUM6,NUM+,NUM1}
  [11] = {NUM2,NUM3,NUM0,NUM.,,,,F11}
  [12] = {F12,,,,,,,}
  [13] = {NUMENTER,RCTRL,NUM/,,RALT,,HOME,UPARR}
  [14] = {PAGEUP,LEFTARR,RIGHTARR,END,DOWNARR,PAGEDOWN,INSERT,DELETE}
  [15] = {,,,,,,,PAUSEBREAK}

  DON'T HAVE: PRTSC, FN, WINDOWKEY
*/
